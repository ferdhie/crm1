...
jQuery.Class("ModuleBuilder_ModuleBuilderView_Js",{
	    ...
	    if(typeof window[moduleClassName] != 'undefined'){
			...
			instance 		= new ModuleBuilder_ModuleBuilderView_Js();
		}
	    return instance;
	},
},{
	...
	makeBlocksListSortable : function() {
		var thisInstance = this;
		var contents 	 = jQuery('#moduleBlocks');
	...
	updateBlockSequence : function() {
		...
		jQuery('#tkssequence').val(sequence);
		/*TECHNOKRAFTS START -- Added to by pass the seq it is handled in later stage*/
		progressIndicatorElement.progressIndicator({'mode' : 'hide'});
		return;
		/*TECHNOKRAFTS END -- Added to by pass the seq it is handled in later stage*/
	...
	createUpdatedBlockFieldsList : function() {
		...
		for(var index in  thisInstance.updatedBlocksList) {
			...
			editFields.each(function(i,domElement){
				...
				thisInstance.updatedBlockFieldsList.push({'fieldid' : fieldId,'sequence' : expectedFieldSequence, 'block' : updatedBlockId, 'lbl' : fieldEle.find('.fieldLabels').text().replace("*","")});
			...
			secondEditFields.each(function(i,domElement){
				var fieldEle 	= jQuery(domElement);
				var fieldId 	= fieldEle.data('fieldId');
				thisInstance.updatedBlockFieldsList.push({'fieldid' : fieldId,'sequence' : sequenceValue, 'block' : updatedBlockId, 'lbl' : fieldEle.find('.fieldLabels').text().replace("*","")});
	...
	registerFieldTypeChangeEvent : function(form) {
		...
		form.find('[name="fieldType"]').on('change', function(e) {
			...
			if( selectedOption.val() == 'Relate')
			{
				var relmod = form.find('.tksrelatedModules');
				relmod.removeClass('hide');
				
				var isfilter = form.find('.isFilter');
				isfilter.addClass('hide');
			}
			else
			{
				var relmod 		= form.find('.tksrelatedModules');
				relmod.addClass('hide');
				var isfilter 	= form.find('.isFilter');
				isfilter.removeClass('hide');
			}
	...
	/**
	 * Function to append the new field label to the entity identifer field
	 */
	appendFieldToIdentifierDropDown : function(result) {
		if(result['mandatory'] == 'true' && result['tksfilterfield'] == 'true') {
			jQuery("#tks_entity").removeAttr( "disabled" );
			if(jQuery("#tks_entity").val()== '')
				jQuery("#tks_entity").append('<option value="'+result['blockid']+':'+result['id']+'" selected="selected">'+result['label']+'</option>');
			else
				jQuery("#tks_entity").append('<option value="'+result['blockid']+':'+result['id']+'">'+result['label']+'</option>');
			
			var dropdown = jQuery("#tks_entity");
			app.destroyChosenElement(dropdown)
			dropdown.addClass('chzn-select');
			app.changeSelectElementView(dropdown);
		}
	},
	
	/**
	 * Function to remove the field label to the entity identifer field
	 */
	removeFieldToIdentifierDropDown : function(result) {
			if(jQuery('#tks_entity').val() == result['blockid']+':'+result['id'] ){
				jQuery('#tks_entity option:selected').removeAttr('selected');
				jQuery('#tks_entity option[value=""]').attr('selected','selected');
			} 
			jQuery('#tks_entity option[value="'+result['blockid']+':'+result['id']+'"]').remove();
	...
	/**
	 * Function to show added new custom field ui to the list
	 */
	showCustomField : function(result) {
		...
		fieldContainer.find('.fieldLabels').text(result['label']);
		
		if(result['mandatory'] == 'true') {
		fieldContainer.find('.fieldLabels').append(jQuery('<span class="redColor">*</span>'));
		}
		
		thisInstance.appendFieldToIdentifierDropDown(result);
	...
	setFieldDetails : function(result, form) {
		...
		if(result['isQuickCreateDisabled'] == 'true') {
			form.find('[name="quickcreate"]').filter(':checkbox').attr('readonly', 'readonly');
		}
		if(result['isSummaryField'] == 'true') {
			form.find('[name="summaryfield"]').filter(':checkbox').attr('checked', true);
		}
		if(result['isSummaryFieldDisabled']== 'true') {
			form.find('[name="summaryfield"]').filter(':checkbox').attr('readonly', 'readonly');
		}
		if(result['masseditable'] == 'true') {
			form.find('[name="masseditable"]').filter(':checkbox').attr('checked', true);
		}
		
		if(result['tksfilterfield'] == 'true') {
			form.find('[name="tksfilterfield"]').filter(':checkbox').attr('checked', true);
		}
		...
		if(result['type'] == 'relate')
		{
			form.find('[name="defaultvalue"]').filter(':checkbox').attr('readonly', 'readonly');
		}
	 ...
	 */
	 registerAddCustomBlockEvent : function() {
		...
		contents.find('.addCustomBlock').click(function(e) {
			...
			mod.validationEngine('validate');
			if( mod.val() == '')
			{
				lbl.validationEngine('hide');
				mod.validationEngine('showPrompt', app.vtranslate('JS_REQUIRED_FIELD') , 'error','topLeft',true);
				mod.focus(function(e) {mod.validationEngine('hide')});
			}
			else if( !mod.val().match(/^[a-zA-Z]+$/) )
			{
				lbl.validationEngine('hide');
				thisInstance.registerValidateShowPrompt(mod);
			}
			else if( lbl.val() == '' )
			{
				mod.validationEngine('hide');
				lbl.validationEngine('showPrompt', app.vtranslate('JS_REQUIRED_FIELD') , 'error','topLeft',true);
				lbl.focus(function(e) {lbl.validationEngine('hide')});
			}
			else if( !lbl.val().match(/^[a-zA-Z\s]+$/) )
			{
				mod.validationEngine('hide');
				thisInstance.registerValidateShowPrompt(lbl);
			}
			else
			{
				f1.validationEngine('hideAll');
	...
	saveBlockDetails : function(form) {
		...
		params['tks_mod'] 	 = jQuery('#tks_modulename').val();
		params['tks_label']  = jQuery('#tks_modulelabel').val();
		params['tks_parent'] = jQuery('#tks_parent').val();
		params['token'] 	 = thisInstance.getToken();
	...
	displayNewCustomBlock : function(result) {
		...
		if( beforeBlockId == 0)
		{
			var beforeBlock = contents.find('.tksfirstblock');
			beforeBlockId = 1;
		}
		else
		{
	...
	updateNewSequenceForBlocks : function(sequenceList) {
		...
		jQuery.each(sequenceList, function(blockId, sequence) {
		    var b = parseInt(blockId) + 1;
		    var seq = parseInt(sequence) + 1;
			contents.find('.block_'+b).attr('data-sequence', seq);
		});
	},
	...
	/**
	 * Function to save and update the block status
	 */
	...
	deleteCustomField : function(fieldId,blockid) {
		...
		AppConnector.request(params).then(
			function(data) {
				progressIndicatorElement.progressIndicator({'mode' : 'hide'});
				var result 	= data['result'];
				if(thisInstance.checkLabelExist('tks_entity',result['label']) == true)
				{
					thisInstance.removeFieldToIdentifierDropDown(result);	
				}
				aDeferred.resolve(data);
			},
			function(error) {
	...
	registerSaveFieldDetailsEvent : function(form) {
		...
		var blockId 		= block.find('blockId');
		//close the drop down
		...
		thisInstance.saveFieldDetails(submitButtton).then(
			function(data) {
				var result 		= data['result'];
				var fieldLabel 	= fieldRow.find('.fieldLabels');
				...
				if(result['mandatory'] == 'true') {
					if(fieldLabel.find('.redColor').length == '0') {
						fieldRow.find('.fieldLabels').append(jQuery('<span class="redColor">*</span>'));
					}
				} else {
					fieldRow.find('.fieldLabels').find('.redColor').remove();
				}
				...
				var entityIdentifier = jQuery('#tks_entity');
				if( result['mandatory'] == 'true' && result['tksfilterfield'] == 'true' && thisInstance.checkLabelExist('tks_entity',result['label']) == false )
				{
					thisInstance.appendFieldToIdentifierDropDown(result);
				}
				
				if( ( result['mandatory'] == 'false' || result['tksfilterfield'] == 'false' ) && thisInstance.checkLabelExist('tks_entity',result['label']) == true )
				{
					thisInstance.removeFieldToIdentifierDropDown(result);
				}
			},
	...
	/**
	 * Function check if label exsist
	 */
	checkLabelExist : function(ids,lbl) {
		var falg = false;
		jQuery("#"+ids+" option").each(function(){
		  if(jQuery(this).text() == lbl){
			  falg = true;
			return;
		  }
		});
	...
	saveFieldDetails : function(currentTarget) {
		...
		var block 				= currentTarget.closest('.blockSortable');
		var blockid				= block.find('#tksblockid'); 
		var params 				= form.serializeFormData();
		params['module'] 		= app.getModuleName();
		params['parent'] 		= app.getParentModuleName();
		params['action'] 		= 'Field';
	...
	/**
	 * Function to edit field deatils attributes on click edit icon
	 */
	...
	checkForDownload : function() {
		...
		AppConnector.request(params).then(
			function(data) {
				...
				{
					params['text'] = app.vtranslate('JS_RE_SAVE');
					params['type'] = 'error';
					Settings_Vtiger_Index_Js.showMessage(params);
				}
			},
	 ...
	 */
	 emptyBlockValidation : function() {
		...
		jQuery.each(contents,function(index,domElement){	
			...
			if(row.find('[name="sortable1"]').text() == '' )
			{
				if( !row.hasClass('hide') )
				{
					status = row.find('#tksblocklabel').val();
					return status;
				}
			}
		});
	...
	buildModuleEvent : function(form){
		...
		{
			var params 		= {};
			params['text'] 	= app.vtranslate('JS_EMPTY_BLOCK_FIELD');
			params['type'] 	= 'error';
			Settings_Vtiger_Index_Js.showMessage(params);
		}
		...
		{
			var params 		= {};
			params['text'] 	= app.vtranslate('JS_BLOCK_EMPTY1')+blockval+app.vtranslate('JS_BLOCK_EMPTY2');
			params['type'] 	= 'error';
			Settings_Vtiger_Index_Js.showMessage(params);
		}
					...
					AppConnector.request(builddata).then(
						function(data) {
							...
							if(data['result']['saved'] == 'success') {
								params['text'] = app.vtranslate('JS_SAVE_MODULE_SUCCESS');
								params['type'] = 'success';
								saveButton.removeClass('hide');
	...
	getSequence : function(){
        var thisInstance = this;
		var seq 		 = {};
		var contents 	 = jQuery('.editFieldsTable');
		jQuery.each(contents,function(index,domElement){	
			var blockTable 	= jQuery(domElement);
			var blockId 	= blockTable.find('#tksblocklabel').val();
			seq[index] 		= blockId;
		});
	 ...
	 /**
	 * Function to trigger Initialize Module Build Event module event
	 */
	registerModuleBuildInitializeEvent : function(){
        var thisInstance = this;
		var contents 	 = jQuery('#editViewContainer');
		var req 		 = app.vtranslate('JS_REQUIRED_FIELD');
		jQuery('#nextStep').click(function(e) {											
			...
			if( mod.val() == '')
			{
				prnt.validationEngine('hide');
				lbl.validationEngine('hide');
				mod.validationEngine('showPrompt', req , 'error','topLeft',true);
				mod.focus(function(e) {mod.validationEngine('hide')});
			}
			else if( !mod.val().match(/^[a-zA-Z]+$/) )
			{
			...
			else if( lbl.val() == '' )
			{
				prnt.validationEngine('hide');
				mod.validationEngine('hide');
				lbl.validationEngine('showPrompt', req  , 'error','topLeft',true);
				lbl.focus(function(e) {lbl.validationEngine('hide')});
			}
			else if( !lbl.val().match(/^[a-zA-Z\s]+$/) )
			{
			...
			else if( prnt.val() == '' )
			{
				lbl.validationEngine('hide');
				mod.validationEngine('hide');
				prnt.validationEngine('showPrompt', req  , 'error','topLeft',true);
				prnt.focus(function(e) {lbl.validationEngine('hide')});
			}
			else
			{
				f1.validationEngine('hideAll');
				...
				AppConnector.request(builddata).then(
					function(data) {
						...
						if(data['result']['init'] == true) {
							...
							params['text'] = app.vtranslate('JS_PLEASE_MODULE_INITIALIZE');
							params['type'] = 'success';
						} else {
	...
	registerModuleBuildSubmitEvent : function(){
        var thisInstance = this;
		jQuery('body').on('submit','#tks_modulevalidate',function(e){
	 ...
	 */
	registerValidateShowPrompt : function(field){ 
		if( !field.val().match(/^[a-zA-Z]+$/) )
		{
			field.validationEngine('showPrompt', app.vtranslate('JS_ALPHABETS_ONLY') , 'error','topLeft',true);
			field.focus(function(e) {field.validationEngine('hide')});
		}	
	},
	 ...
	 */
	registerToolTipsEvent : function(field){ 
		var tooltipHtml = jQuery('.tooltiphide').html();
		jQuery(function () { 
	...
	/**
	 * Function to validate module name and module label
	 */
	registerModuleNameAndLabelValidateEvent : function(){
        var thisInstance = this;
		contents 		 = jQuery('.editViewContainer');
		contents.find('#tks_modulename').change(function(e) {
		...
		contents.find('#tks_modulelabel').change(function(e) {
			var form 	= jQuery('.tks_modulevalidate');
			var params 	= app.validationEngineOptions;
			form.validationEngine(params);
			form.submit(function(e) {
...
jQuery(document).ready(function() {
	var instance = ModuleBuilder_ModuleBuilderView_Js.getInstanceByView();
	instance.registerEvents()
	ModuleBuilder_ModuleBuilderView_Js.currentInstance = instance;
	var cnt 	 = jQuery('#modulename').focus();
...
Vtiger_WholeNumberGreaterThanZero_Validator_Js("Vtiger_DecimalMaxLength_Validator_Js",{
	...
	validate: function(){
		...
		if(response != true){
			...
			if( parseInt(decimalFieldValue) > parseInt(fieldValue) )
			{
				var errorInfo 		= app.vtranslate('JS_LENGTH_SHOULD_BE_GREATER_THAN_EQUAL_TO');
				this.setError(errorInfo);
				return false;
			}
			var specialChars 		= /^[+]/ ;
...
Vtiger_Base_Validator_Js("Vtiger_FieldLabel_Validator_Js",{
	...
	validateValue : function(fieldValue){
		...
		{
			var errorInfo = app.vtranslate('JS_ALPHABETS_ONLY');
			this.setError(errorInfo);
			return false;
		}
		else if (specialChars.test(fieldValue)) {
...
Vtiger_Base_Validator_Js("Vtiger_OnlyAphabets_Validator_Js",{
	 ...
	 */
	invokeValidation: function(field, rules, i, options){
		var instance 	= new Vtiger_OnlyAphabets_Validator_Js();
		instance.setElement(field);
		var response 	
		= instance.validate();
		if(response != true){
...
Vtiger_Base_Validator_Js("Vtiger_OnlyAphabetsWithSapce_Validator_Js",{
	 ...
	 */
	invokeValidation: function(field, rules, i, options){
		var instance = new Vtiger_OnlyAphabetsWithSapce_Validator_Js();
		instance.setElement(field);
		var response = instance.validate();
		if(response != true){
			return instance.getError();
