...
{strip}
	...
	{assign var="cf_914" value="cf_914"|cat:$row_no}
	...
	<td>
		...
		{else}
                        <div><br><textarea id="{$comment}" name="{$comment}" class="lineItemCommentBox">{$data.$comment}</textarea></div>
		{/if}
	</td>
	<td>
	...
	<td>
		<div>
			...
			{*$PRICEBOOK_MODULE_MODEL|var_dump*}
			{if $PRICEBOOK_MODULE_MODEL->isPermitted('DetailView')}
				<img src="{vimage_path('PriceBooks.png')}" class="cursorPointer alignMiddle priceBookPopup" data-popup="Popup" data-caller="SalesOrder" data-module-name="PriceBooks" title="{vtranslate('PriceBooks',$MODULE)}"/>
			{/if}
		</div>
		<div>
			<span>
