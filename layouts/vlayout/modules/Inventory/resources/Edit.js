...
Vtiger_Edit_Js("Inventory_Edit_Js",{
	...
	/**
	 * Function write modification input field
	 *
	 **/
	getLPValue : function(lineItemRow) {
		return parseFloat(jQuery('.lp',lineItemRow).val());
	},

	setLPValue : function(lineItemRow, lpValue) {
		var numberOfDecimal = parseInt(jQuery('.numberOfCurrencyDecimal').val());
		var lp = parseFloat(lpValue).toFixed(numberOfDecimal);
		lineItemRow.find('.lp').val(lp);
		return this;
	},

	setBookingFeeValue : function(bookingfeeValue){
		var numberOfDecimal = parseInt(jQuery('.numberOfCurrencyDecimal').val());
		var bookingfee = parseFloat(bookingfeeValue).toFixed(numberOfDecimal);
		jQuery('#bookingfee').val(bookingfee);
		return this;
	},

	getBookingFeeValue : function(lineItemRow) {
		return parseFloat(jQuery('#bookingfee',lineItemRow).val());
	},

	setNupValue : function(nupValue){
		var numberOfDecimal = parseInt(jQuery('.numberOfCurrencyDecimal').val());
		var nup = parseFloat(nupValue).toFixed(numberOfDecimal);
		jQuery('#nup').val(nup);
		return this;
	},
	...
	pricebooksPopupHandler : function(popupImageElement){
		...
		this.showPopup(params).then(function(data){
                   var responseData = JSON.parse(data);
                   thisInstance.setListPriceValue(lineItemRow,parseFloat(responseData[0]));
	...
	lineItemToTalResultCalculations : function(){
                console.log('Inventory_Edit_JS.lineItemToTalResultCalculations');
