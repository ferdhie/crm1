...
{strip}
	...
	<div class="row-fluid verticalBottomSpacing" {if $MODULE eq "SalesOrder"}style="display:none"{/if}>
		<div>
...
{if $MODULE eq "SalesOrder"}<br>{/if}
	...
	<br>
	<table class="table table-bordered blockContainer lineItemTable">
		<tr>
			<th><b>Keterangan</b></th>
			<th><b>Jatuh Tempo</b></th>
			<th><b class="pull-right">Jumlah</b></th>
		</tr>
		<tr>
			<td>Nomor Unit Pesanan / NUP</td>
			<td>
				<span class="input-append date pull-right">
					<input id="nup_due_date" name="nup_due_date" type="text" class="dateField input-small" data-date-format="{$USER_MODEL->get('date_format')}" value="{if $FINAL.nup_due_date}{DateTimeField::convertToUserFormat($FINAL.nup_due_date)}{/if}"/>
					<span class="add-on"><i class="icon-calendar"></i></span>
				</span>
			</td>
			<td><input id="nup" name="nup" type="text" class="pull-right sonum" value="{if $FINAL.nup}{$FINAL.nup}{else}0{/if}"/></td>
		</tr>
		<tr>
			<td>Booking Fee</td>
			<td>
				<span class="input-append date pull-right">
					<input id="bookingfee_due_date" name="bookingfee_due_date" type="text" class="dateField input-small" data-date-format="{$USER_MODEL->get('date_format')}" value="{if $FINAL.bookingfee_due_date}{DateTimeField::convertToUserFormat($FINAL.bookingfee_due_date)}{/if}"/>
					<span class="add-on"><i class="icon-calendar"></i></span>
				</span>
			</td>
			<td><input id="bookingfee" name="bookingfee" type="text" class="pull-right sonum" value="{if $FINAL.bookingfee}{$FINAL.bookingfee}{else}0{/if}"/></td>
		</tr>
                {if $FINAL.jangka_waktu_um < 1}
                    {assign "jangka_waktu_um" 1}
                {else}
                    {assign "jangka_waktu_um" $FINAL.jangka_waktu_um}
                {/if}
		<tr>
			<td>Jangka Waktu UM</td>
			<td>
                            <select id="jangka_waktu_um" name="jangka_waktu_um" class="input-medium pull-right" placeholder="Termin Uang Muka">
                                <option value="">Termin Uang Muka</option>
                                {for $i=1 to 36}
                                <option value="{$i}" {if $jangka_waktu_um eq $i}selected{/if}>{$i}x Pembayaran</option>
                                {/for}
                            </select>
                        </td>
                        <td>&nbsp;</td>
                ...
                {for $i=1 to 36}
		<tr class="um {if $i >= $jangka_waktu_um}hide{/if}">
			<td>Uang Muka {$i} {if $i eq 1}/ DP{/if}</td>
			<td>
				<span class="input-append date pull-right">
					<input id="um{$i}_due_date" name="um{$i}_due_date" type="text" class="dateField input-small um_date" data-date-format="{$USER_MODEL->get('date_format')}" value="{if $uangmuka[$i-1].um_due_date}{DateTimeField::convertToUserFormat($uangmuka[$i-1].um_due_date)}{/if}"/>
					<span class="add-on"><i class="icon-calendar"></i></span>
				</span>
			</td>
			<td><input id="um{$i}" name="um{$i}" type="text" class="pull-right sonum" value="{if $uangmuka[$i-1].um}{$uangmuka[$i-1].um}{else}0{/if}"/></td>
		</tr>
                {/for}
		<tr>
			<td>JAMSOSTEK</td>
			<td>
				<span class="input-append date pull-right">
					<input id="jamsostek_due_date" name="jamsostek_due_date" type="text" class="dateField input-small" data-date-format="{$USER_MODEL->get('date_format')}" value="{if $FINAL.jamsostek_due_date}{DateTimeField::convertToUserFormat($FINAL.jamsostek_due_date)}{/if}"/>
					<span class="add-on"><i class="icon-calendar"></i></span>
				</span>
			</td>
			<td><input id="jamsostek" name="jamsostek" type="text" class="pull-right sonum" value="{if $FINAL.jamsostek}{$FINAL.jamsostek}{else}0{/if}"/></td>
		</tr>
		<tr>
			<td>
				<select class="select2 input-medium" id="carapinjam" name="carapinjam">
			...
			<td>
				<span class="input-append date pull-right">
					<input id="pinjaman_due_date" name="pinjaman_due_date" type="text" class="dateField input-small" data-date-format="{$USER_MODEL->get('date_format')}" value="{if $FINAL.pinjaman_due_date}{DateTimeField::convertToUserFormat($FINAL.pinjaman_due_date)}{/if}"/>
					<span class="add-on"><i class="icon-calendar"></i></span>
				</span>
			</td>
			<td><input id="pinjaman" name="pinjaman" type="text" class="pull-right sonum" value="{if $FINAL.pinjaman}{$FINAL.pinjaman}{else}0{/if}"/></td>
		</tr>
		<tr>
			<td colspan="2"><div align="right"><b>Total Pembayaran</b></div></td>
			<td><input readonly="readonly" id="totalbayar" name="totalbayar" type="text" class="pull-right sonum" value="{if $FINAL.totalbayar}{$FINAL.totalbayar}{else}0{/if}"/></td>
		</tr>

	</table>
                
        <input type="hidden" id="angsuran1" name="angsuran1" value="{$FINAL.angsuran1}"/>
        <input type="hidden" id="angsuran2" name="angsuran2" value="{$FINAL.angsuran2}"/>
        <input type="hidden" id="angsuran3" name="angsuran3" value="{$FINAL.angsuran3}"/>
